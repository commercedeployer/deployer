...
| **Сейф** (`$DEPLOY_BASE_PATH/secrets.json`) | инфра-секреты: `POSTGRES_ADMIN_URL`, `COMMERCE_SALT`, … — в шаблоне только `{{KEY}}` |
| stdout provision (`expect`) | tenant-креды → `{{DB_USER}}` в env контейнера |

### Порядок подстановки `{{KEY}}`

1. params deploy (форма + `containerName` в том же запросе)  
2. outputs provision (`DB_USER`, `SECRET`, …)  
3. сейф (`secrets.json`)  
4. env контейнера Deployer (оставшиеся `{{KEY}}`)
...
`psql` и Postgres должны быть доступны **из контейнера Deployer** (в setup-server-stack — хост `postgres:5432` в Docker-сети; admin URL — в шаблоне, не `127.0.0.1` на Ubuntu). Пакет `psql` — ключ **`psql`** в `DEPLOYER_SOFTWARE`. При `ENABLE_POSTGRES=1` Postgres на том же VPS в compose.
